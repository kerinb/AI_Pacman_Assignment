...
class SearchAgent(Agent):
    ...
    def registerInitialState(self, state):
        ...
        print('Path found with total cost of %d in %.20f seconds' % (totalCost, time.time() - starttime))
...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        """
        successors_list = []
        for direction_action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            x, y = state[0]
            dx, dy = Actions.directionToVector(direction_action)
            nextx, nexty = int(x + dx), int(y + dy)
            ...
            if not self.walls[nextx][nexty]:
                ...
                if next_pos in state[1]:
                    ...
                    successors_list.append(((next_pos, next_corners), direction_action, 1))
                else:
                    successors_list.append(((next_pos, state[1]), direction_action, 1))
...
def cornersHeuristic(state, problem):
    ...
    while corners:
        ...
        for x1, y1 in corners:
            distances_to_corners.append(euclidean(x1, y1, x2, y2))
        heuristic += min(distances_to_corners)
        curr_pos = corners[distances_to_corners.index(min(distances_to_corners))]
